...
const sequence = applicativeReturningFn => iterable => {
  ...
  if (!isApplicative(initApplicativeValue)) {
    throw new Error(`Make sure that the applicativeReturningFn you pass in to sequence returns a value which has .map, .ap, and .of methods as specified in the Fantasy Land spec. This value failed the test: ${initApplicativeValue}`)
  }
